...
**システム規模**:
- 画面数: 15画面（認証1 + 生徒4 + 担任3 + 管理者7）
- ユーザー数: 187名（管理者1 + 担任6 + 生徒180）
- クラス数: 6クラス（1〜3年各2クラス）
- テストデータ: 連絡帳約550件
...
| **AWS** | ・既存ポートフォリオのリソース活用でコスト削減<br>・スケーラビリティ確保<br>・ACMによるSSL証明書の自動更新 |
...
## 6. 課題２のプレゼンテーション
課題１の現行仕様に対して、より現場で使いやすくするため、下記の項目を提案いたします。

### 提案項目一覧
| No | 提案 | 目的 | 技術実装方法 | UI変更 | DB変更 | 実装 |
|----|------|------|------------|--------|--------|:----:|
| **No1: データ可視化** |
| 1-1 | 個別生徒の推移グラフ | 生徒の状態変化を時系列で把握、早期異変検知 | Chart.js + Laravelコントローラーで集計 | ✅ 追加 | ❌ なし | ✅ 実装 |
| 1-2 | クラス全体の統計グラフ | 受験期・テスト期の全体傾向把握 | Chart.js（グラフ描画） + Laravel（データ集計） | ✅ 追加 | ❌ なし | 📄 提案のみ |
| 1-3 | 担任ダッシュボード強化 | 注意が必要な生徒リストの表示 | Eloquent where条件(2以下) + Blade | ✅ 修正 | ❌ なし | 📄 提案のみ |
| **No2: 情報共有・フィードバック** |
| 2-1 | 気になる生徒のフラグ機能 | 担任間でタイムリーな情報共有 | 連絡帳テーブルにflagカラム追加 | ✅ 追加 | ✅ カラム追加 | ✅ 実装 |
...
| **No3: 役割拡張** |
| 3-1 | 学年主任ロール追加 | 学年全体の把握、クラス比較 | ロール追加 + 権限管理クラス（誰が何を見られるか制御） | ✅ 追加 | ✅ カラム修正 | 📄 提案のみ |
| 3-2 | 副担任ロール追加 | 複数教師での見守り | usersテーブルroleカラム値追加 | ✅ 追加 | ✅ カラム修正 | 📄 提案のみ |
| 3-3 | 学年主任ダッシュボード | 学年全体の統計表示 | 新規ビュー + 学年全体集計 | ✅ 追加 | ❌ なし | 📄 提案のみ |
| **No4: アラート機能** |
| 4-1 | 体調異常の自動検知 | 早期の異変検知 | 毎日自動チェック + システム内通知（ダッシュボード表示） | ✅ 追加 | ✅ テーブル追加 | 📄 提案のみ |
| 4-2 | 提出率低下アラート | クラス単位の問題検知 | 集計バッチ + 閾値判定 | ✅ 追加 | ❌ なし | 📄 提案のみ |
| **No5: モバイル対応** |
| 5-1 | レスポンシブ改善 | スマホでの操作性向上 | Tailwind CSSブレークポイント調整 | ✅ 修正 | ❌ なし | 📄 提案のみ |
| **No6: 運用ルール** |
| 6-1 | 運用ルール整備 | 実運用での混乱防止 | マニュアル・ルール文書作成 | ❌ なし | ❌ なし | 📄 提案のみ |
