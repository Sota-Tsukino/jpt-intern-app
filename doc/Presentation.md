...
#### 課題２での拡張
- **複数スタンプ対応**: `stamp_type`カラム追加（good/great/fighting/care）
- **教師フィードバック**: `teacher_feedback`カラム、`feedback_by`カラム追加
- **フラグ機能（DB準備）**: `flag`、`flagged_at`、`flagged_by`、`flag_memo`カラム追加
- **副担任ロール**: `users`テーブルの`role`にsub_teacher値を追加
...
**実装を見送った機能について**:
- **2-1（気になる生徒のフラグ機能）**: 時間制約により提案のみ。データベーステーブル・カラムは準備済みのため、将来的な実装が可能。
- **2-4（担任間共有メモ機能）**: 時間制約により提案のみ。
