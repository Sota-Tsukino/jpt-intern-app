...
- エラー対応に予想以上の時間を要した
  - ログファイルの書き込み権限によるエラー
  - apacheの設定ファイルによるエラー（ルートドキュメント設定）
...
**学びと成果**:
- 事前に自身のオリジナルポートフォリオ制作でも同様の設計から実装、デプロイまで一通りの学習をしていたので、インターン課題で活かすことができてよかった。
- Claude Codeを使用することで効率よく実装作業を進めることができたと思う。特に要件定義書を作り込むことで、意図通りの設計仕様に近い実装ができた。

**課題と今後の改善点**:
- ASD（アスペルガー症候群）の傾向によるこだわりが強い特性があるため、１００時間という制約の中で優先順位の付け方や時間の使い方などタイムマネジメントが難しかった。
- 課題２の提案では、時間制約により一部資料の作り込みができなかったことが残念。
