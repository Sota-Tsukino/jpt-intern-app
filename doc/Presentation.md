...
1. [プロジェクト概要](#1-プロジェクト概要)
   ...
   - 1.4 [成果物の概要](#14-成果物の概要)
2. [機能要件](#2-機能要件)
   - 2.1 [生徒機能](#21-生徒機能)
   - 2.2 [担任機能](#22-担任機能)
   - 2.3 [管理者機能](#23-管理者機能)
   - 2.4 [画面遷移図](#24-画面遷移図)
3. [明記されていない要件への対応](#3-明記されていない要件への対応)
4. [課題１の工夫点・アピール](#4-課題１の工夫点アピール)
   - 4.1 [要件達成度](#41-要件達成度)
   - 4.2 [技術スタックと選定理由](#42-技術スタックと選定理由)
   - 4.3 [セキュリティへの配慮](#43-セキュリティへの配慮)
   - 4.4 [データベース設計の工夫](#44-データベース設計の工夫)
   - 4.5 [機能実装の工夫](#45-機能実装の工夫)
   - 4.6 [開発プロセスの工夫](#46-開発プロセスの工夫)
5. [課題２のプレゼンテーション](#5-課題２のプレゼンテーション)
   - 5.1 [提案項目一覧](#51-提案項目一覧)
   - 5.2 [権限マトリクス](#52-権限マトリクス)
   - 5.3 [拡張版ER図](#53-拡張版er図)
   - 5.4 [実装済み機能の詳細](#54-実装済み機能の詳細)
   - 5.5 [未実装機能の提案](#55-未実装機能の提案)
6. [課題・インターンの感想](#6-課題インターンの感想)
   - 6.1 [技術的な課題](#61-技術的な課題)
   - 6.2 [プロジェクト管理の学び](#62-プロジェクト管理の学び)
   - 6.3 [反省点](#63-反省点)
   - 6.4 [感想](#64-感想)
...
| 1-3 | 担任ダッシュボード強化 | 注意が必要な生徒リストの表示 | Eloquent where条件（体調・メンタル2以下） + Blade | ✅ 修正 | ❌ なし | ✅ 実装 |
| **No2: 情報共有・フィードバック** |
...
| **No4: アラート機能** |
| 4-1 | 体調異常の自動検知 | 早期の異変検知 | 毎日自動チェック + システム内通知（ホーム画面表示） | ✅ 追加 | ✅ テーブル追加 | 📄 提案のみ |
| 4-2 | 提出率低下アラート | クラス単位の問題検知 | 集計バッチ + 閾値判定 | ✅ 追加 | ❌ なし | 📄 提案のみ |
| **No5: モバイル対応** |
| 5-1 | レスポンシブ改善 | スマホでの操作性向上 | Tailwind CSSブレークポイント調整  | ✅ 修正 | ❌ なし | 〇 一部実装 |
---
...
**画面設計図**:
![課題2_画面設計図](images/Screen_Proposal2.png)
...
**この機能の特徴**:
- ユースケースでは、体調・メンタルが２以下の場合などに担任・副担任ロールが注意フラグを設定することを想定
...
- 学年全体の統計グラフ表示
- クラス間の比較機能
- ※　時間制約により、画面設計図の作成が間に合わなかったため、簡易提案としております。
...
- 提出率低下アラート
- ※ 時間制約により技術課題の掘り下げが未完了のため、簡易提案としております。
  
#### モバイル対応について
- モバイル対応は、現状はPoCなので優先順位を落とし、簡易的に実装を施した（一部の画面で対応してない画面あり）
- 今後も現場からのフィードバックや仕様変更に伴い、画面レイアウト図が変更になる可能性があるため、優先順位を下げて実装を進めた

## 6. 課題・インターンの感想
今回のインターンでは、要件定義や画面・データベース設計、実装、デプロイ、仕様書のまとめまで、一通り網羅するような課題でなおかつ１００時間以内の制約の中でいかに優先順位付けをして課題を進めていくかで非常に難しい課題でしたが、自身の学びにもなった点が沢山あったので非常にやりがいのある課題で面白かったです。

### 6.1 技術的な課題

**AWSインフラ構築の難しさ**

当初の想定では10時間で計画してましたが、エラー対応に想定より時間がかかってしまい17時間ほどかかってしまいました。

**エラーの例**
- ファイルの書き込み権限の設定ミスによるログファイルの書き込みエラー
- サーバーソフトウェアApacheの設定ファイルによるルートドキュメント設定エラー

### 6.2 プロジェクト管理の学び

自信が各タスクごとの時間配分が苦手なため、何を優先して何を捨てるかなど優先順位付けの大切さを再認識させられました。
...
### 6.3 反省点
自信がASD傾向があり、こだわりが強い特性を持っている為、下記で時間のロスをしてしまった印象があります。
...
事前に自身のオリジナルポートフォリオ制作でも同様の設計から実装、デプロイまで一通りの学習をしていたので、インターン課題で活かすことができてよかったです。
また、今回はAIツールのClaude Codeを初めて使用し、要件定義書を詳細に作り込むことで意図通りの設計仕様に近い内容で効率よく実装作業を進めることができたと思います。

**課題と今後の改善点**:

ASD（アスペルガー症候群）の傾向によるこだわりが強い特性があるため、１００時間という制約の中で優先順位の付け方や時間の使い方などタイムマネジメントが難しかったです。特に課題２の提案では、時間制約により一部資料の作り込みができなかったことが残念でした。今後も引き続き優先順位づけや時間の使い方などを意識していきたいと思います。ありがとうございました。
